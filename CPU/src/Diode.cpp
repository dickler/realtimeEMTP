...
void Diode::stampConductance(Circuit &circuit){
    //Creates temporary variables for improved readability
    double StampValue;
    double Stmp;
    int n = 0, nn = 0;
...
void Diode::stampPejovic(Circuit &circuit){
    //Creates temporary variables for improved readability
    double StampValue;
    double Stmp;
    int n = 0, nn = 0;
...
void Diode::stampMANA(Circuit &circuit){
...
void Diode::idealStamp(Circuit& circuit) {//Overrides the stamp fucntion to the Diode stamp for FAMNM
...
void Diode::calculateState(){
    ...
    case 1:
        if(this->S){
            this->S =this->Ih - this->value*(*(Voltage[0]) - *(Voltage[1]))<0;
        }
        else{
            this->S = this->value*(*(Voltage[0]) - *(Voltage[1]))>0 ;
        }
        break;
    ...
    case 2:
        if(this->S){
            this->S = this->parentCircuit->SolvedVector[parentCircuit->numNodes + ID - 1]<0;
